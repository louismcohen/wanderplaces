...
const getItemType = (type, info) => {
  switch (type) {
    ...
    case 'collection':
    case 'category':
      return <CategoryDetails info={info} />;
    case 'tag':
  }
};
...
export const ListItem = ({ info, type, navigation, navigateTo }) => {
  return (
    <TouchableOpacity
      ...
      onPress={() => navigation.navigate(navigateTo, info)}
    >
      <View style={styles.leftContainer}>{getItemType(type, info)}</View>
...
export const ListItemContainer = ({ content, navigation, navigateTo }) => {
  return (
    <TouchableOpacity
      ...
      onPress={() => navigation.navigate(navigateTo, info)}
    >
      <View style={styles.leftContainer}>{content}</View>
...
export const PlaceItem = ({ info, navigation, navigateTo }) => {
  ...
  const thisCollection = collections.find(
    (collection) => collection._id === info.collection_id
  );
  ...
  useEffect(() => {
    const getPhoto = async () => {
      ...
      const photo = await getPlacePhotoUri({
        name: info.photos[0].name,
        maxHeight: 240,
      });
      setPhotoUri(photo);
    };
...
const PlaceInfo = ({ info }) => {
  ...
  if (rating) {
    ...
    (itemInfoString.length - 1) % 2 == 0 && itemInfoString.length > 1
      ? itemInfoString.push('\n')
      : itemInfoString.push(`${separator}`);
...
export const CategoryDetails = ({ info }) => {
  return (
    <View
      style={{
        flexDirection: 'row',
    ...
    >
      <View style={styles.detailsContainer}>
        ...
        <Text style={styles.placeNote}>8 Places</Text>
      </View>
    </View>
  );
};
